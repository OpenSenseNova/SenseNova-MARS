...
class TestProcessValidationMetrics(unittest.TestCase):
    ...
    def test_process_validation_metrics_basic(self):
        ...
        # Check that mean@2 is present for source1/score (2 samples for prompt1)
        ...
        # Check that pass@2 (deterministic best-of-n) is present
        self.assertIn("pass@2", result["source1"]["score"])
        self.assertAlmostEqual(result["source1"]["score"]["pass@2"], 0.9)  # max(0.8, 0.9)

        # Check that bootstrap metrics are present
        self.assertIn("std@2", result["source1"]["score"])
        self.assertIn("best@2/mean", result["source1"]["score"])
        self.assertIn("best@2/std", result["source1"]["score"])
        self.assertIn("worst@2/mean", result["source1"]["score"])
        self.assertIn("worst@2/std", result["source1"]["score"])

    def test_process_validation_metrics_with_pred(self):
        """Test process_validation_metrics with prediction data for majority voting."""
        ...
        # Check that mean@3 and pass@3 are present for 3 samples
        self.assertIn("mean@3", result["source1"]["score"])
        self.assertIn("pass@3", result["source1"]["score"])

        # mean@3 = (0.8 + 0.9 + 0.7) / 3 = 0.8
        self.assertAlmostEqual(result["source1"]["score"]["mean@3"], 0.8)

        # pass@3 = max(0.8, 0.9, 0.7) = 0.9
        self.assertAlmostEqual(result["source1"]["score"]["pass@3"], 0.9)

        # Check that bootstrap metrics are present
        self.assertIn("std@3", result["source1"]["score"])
        self.assertIn("best@2/mean", result["source1"]["score"])
        self.assertIn("best@3/mean", result["source1"]["score"])
        self.assertIn("worst@2/mean", result["source1"]["score"])
        self.assertIn("worst@3/mean", result["source1"]["score"])

        # Check that majority voting metrics are present (because pred exists)
        self.assertIn("maj@2/mean", result["source1"]["score"])
        self.assertIn("maj@3/mean", result["source1"]["score"])
