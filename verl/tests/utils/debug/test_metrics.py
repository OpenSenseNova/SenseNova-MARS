...
class TestMetrics(unittest.TestCase):
    def test_calculate_debug_metrics(self):
        ...
        # Verify KL metrics exist and are non-negative (K3 formula is always >= 0)
        assert "training/vllm_k3_kl_mean" in metrics
        assert "training/vllm_k3_kl_max" in metrics
        assert "training/vllm_k3_kl_std" in metrics
        assert metrics["training/vllm_k3_kl_mean"] >= 0
        assert metrics["training/vllm_k3_kl_max"] >= 0
        assert metrics["training/vllm_k3_kl_std"] >= 0
