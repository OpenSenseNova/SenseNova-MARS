...
class RewardManagerWorker:
    ...
    def get_and_reset_judge_stats(self) -> dict:
        """Get current judge LLM stats and reset counters.

        Returns:
            dict with keys: judge_llm_total, judge_llm_successful, judge_llm_success_rate
        """
        from verl.utils.reward_score.tool import get_judge_llm_stats
        return get_judge_llm_stats(reset=True)
