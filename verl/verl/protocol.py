...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        # Handle non_tensor_batch with potentially different keys across chunks
        # (e.g., different datasets may have different reward_extra_keys)
        all_keys = set()
        for d in data:
        ...
        for key in all_keys:
            ...
            for d in data:
                if key in d.non_tensor_batch:
                    ...
                    # Fill missing keys with None array of appropriate size
                    size = len(d)
                    none_array = np.empty(size, dtype=object)
                    none_array[:] = None
                    arrays_to_concat.append(none_array)
        ...
        if data:
            ...
            for d in data:
                for k, v in d.meta_info.items():
                    if k == "metrics":
                        ...
                        # Merge reward_extra_keys as union (different datasets may have different keys)
                        if v is not None:
            ...
            # Store merged reward_extra_keys as sorted list for consistency
            if all_reward_extra_keys:
                merged_meta_info["reward_extra_keys"] = sorted(all_reward_extra_keys)
