...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        # Optional batch normalization (ReBN): apply batch-level z-score after group normalization
        if config is not None and getattr(config, "apply_batch_norm", False):
            # Ensure contiguous memory layout before CUDA reduction kernels
            scores = scores.contiguous()
            # Compute stats in float32 to prevent BFloat16 precision loss in std() computation
            # (variance = mean(x²) - mean(x)² can go negative with low precision)
            batch_mean = scores.float().mean().to(scores.dtype)
            # Use population std (unbiased=False) and epsilon=1e-9 to match GEM ReBN
            batch_std = scores.float().std(unbiased=False).to(scores.dtype)
            scores = (scores - batch_mean) / (batch_std + 1e-9)
...
def compute_grpo_vectorized_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        if config is not None and getattr(config, "apply_batch_norm", False):
            ...
            # Compute stats in float32 to prevent BFloat16 precision loss in std() computation
            # (variance = mean(x²) - mean(x)² can go negative with low precision)
            batch_mean = scalars.float().mean().to(scalars.dtype)
            # Use population std (unbiased=False) and epsilon=1e-9 to match GEM ReBN
            batch_std = scalars.float().std(unbiased=False).to(scalars.dtype)
            scalars = (scalars - batch_mean) / (batch_std + 1e-9)
...
def compute_reinforce_plus_plus_outcome_advantage(
    ...
    **kwargs
    ...
    Args:
        ...
        config: AlgoConfig with gamma, normalization_level, apply_batch_norm
    ...
    Note:
        normalization_level controls aggregation:
        - "token": Per-token discounted returns (original REINFORCE++)
        - "sequence": Sum raw rewards to sequence level (like GRPO/GEM ReBN)

        apply_batch_norm controls normalization:
        - False: Use masked_whiten (token) or raw scores (sequence)
        - True: Apply z-score normalization
    """
    assert config is not None
    gamma = config.gamma
    normalization_level = getattr(config, "normalization_level", "token")
    apply_batch_norm = getattr(config, "apply_batch_norm", False)

    if normalization_level not in ("token", "sequence"):
        raise ValueError(f"Invalid normalization_level: {normalization_level}. Must be 'token' or 'sequence'.")

    with torch.no_grad():
        if normalization_level == "sequence":
            # Sequence-level: sum RAW rewards (matches GRPO/GEM, NO gamma)
            returns_seq = (token_level_rewards * response_mask).sum(dim=-1)  # (bs,)

            if apply_batch_norm:
                # ReBN: z-score normalize at sequence level
                seq_token_counts = response_mask.sum(dim=-1)
                valid_seq_mask = seq_token_counts > 0
                ...
                valid_returns = returns_seq[valid_seq_mask]
                # Ensure contiguous for CUDA kernels (matches GRPO implementation)
                valid_returns = valid_returns.contiguous()
                # Use float32 for numerical stability
                mean_return = valid_returns.float().mean().to(returns_seq.dtype)
                # Handle single valid sequence case (std would be 0)
                # Use population std (unbiased=False) and epsilon=1e-9 to match GEM ReBN
                if valid_returns.numel() > 1:
                    std_return = valid_returns.float().std(unbiased=False).to(returns_seq.dtype)
                else:
                    std_return = torch.tensor(1.0, dtype=returns_seq.dtype, device=returns_seq.device)

                normalized_returns = torch.zeros_like(returns_seq)
                normalized_returns[valid_seq_mask] = (valid_returns - mean_return) / (std_return + 1e-9)
                advantages = normalized_returns.unsqueeze(-1) * response_mask
            else:
                # No normalization, just broadcast raw sequence returns
                advantages = returns_seq.unsqueeze(-1) * response_mask

            # Returns for logging (broadcast sequence returns to token level)
            returns = returns_seq.unsqueeze(-1).expand_as(token_level_rewards) * response_mask

        else:  # normalization_level == "token" (default)
            # Token-level: compute gamma-discounted returns
            returns = torch.zeros_like(token_level_rewards)
            running_return = 0
            ...
            if apply_batch_norm:
                # Token-level batch z-score normalization
                valid_mask = response_mask.bool()
                valid_returns = returns[valid_mask]
                if valid_returns.numel() > 1:
                    valid_returns = valid_returns.contiguous()
                    mean_return = valid_returns.float().mean().to(returns.dtype)
                    # Use population std (unbiased=False) and epsilon=1e-9 to match GEM ReBN
                    std_return = valid_returns.float().std(unbiased=False).to(returns.dtype)
                    advantages = ((returns - mean_return) / (std_return + 1e-9)) * response_mask
                elif valid_returns.numel() == 1:
                    # Single valid token: subtract mean only (std=1.0)
                    mean_return = valid_returns.float().mean().to(returns.dtype)
                    advantages = (returns - mean_return) * response_mask
                else:
                    advantages = torch.zeros_like(returns)
            else:
                # Original REINFORCE++: masked_whiten (z-score over valid tokens)
