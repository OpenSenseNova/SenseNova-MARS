...
def compute_data_metrics(
    batch: DataProto, use_critic: bool = True, reward_extra_infos_keys: list[str] | None = None
) -> dict[str, Any]:
    """
    ...
    Args:
        ...
        reward_extra_infos_keys: List of keys in non_tensor_batch to compute per-dataset and
            global statistics for. If None, no extra reward metrics are computed.
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - reward/{src}/{key}/mean: Per-dataset mean for each key (if reward_extra_infos_keys)
            - reward_info/{key}/mean,max,min: Global statistics for each key (if reward_extra_infos_keys)
    """
    ...
    # Per-dataset breakdown and global aggregates for reward extra info keys
    if reward_extra_infos_keys and "data_source" in batch.non_tensor_batch:
        # Build list of keys to process: "reward" (from tensor) + extra keys (excluding "score")
        keys_to_process = ["reward"] + [k for k in reward_extra_infos_keys if k not in ("score", "reward")]

        for key in keys_to_process:
            # Get values for this key
            if key == "reward":
                values = sequence_reward.tolist()
            ...
            # Group by data source for per-dataset breakdown
            by_source = defaultdict(list)
            for src, val in zip(batch.non_tensor_batch["data_source"], values):
                if val is not None and not (isinstance(val, float) and np.isnan(val)):
                    by_source[src].append(val)

            # Log per-dataset mean
            for src, vals in by_source.items():
                if vals:
                    metrics[f"reward/{src}/{key}/mean"] = np.mean(vals)

    # Global aggregates for each extra info key
    if reward_extra_infos_keys:
        for key in reward_extra_infos_keys:
            if key == "score" or key not in batch.non_tensor_batch:
                continue

            values = batch.non_tensor_batch[key]
            try:
                if hasattr(values, "dtype"):
                    tensor = torch.tensor(values.astype(float))
                elif isinstance(values, (list, tuple)):
                    valid_vals = [float(x) for x in values if x is not None]
                    if not valid_vals:
                        continue
                    tensor = torch.tensor(valid_vals)
                else:
                    continue

                tensor = tensor[~torch.isnan(tensor)]
                if tensor.numel() > 0:
                    metrics[f"reward_info/{key}/mean"] = tensor.mean().item()
                    metrics[f"reward_info/{key}/max"] = tensor.max().item()
                    metrics[f"reward_info/{key}/min"] = tensor.min().item()
...
def process_validation_metrics(
    ...
    Returns:
        ...
        - "pass@N": Max value across N samples (deterministic best-of-n)
        ...
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                if isinstance(var_vals[0], str):
                    continue
                # Filter out None values, tracking valid indices for pred alignment
                valid_indices = [i for i, v in enumerate(var_vals) if v is not None]
                var_vals = [var_vals[i] for i in valid_indices]
                if not var_vals:
                    continue
                ...
                if n_resps > 1:
                    # Deterministic pass@N (actual max) - only when N > 1 (otherwise same as mean)
                    ...
                    for n in ns:
                        ...
                        if var2vals.get("pred", None) is not None:
                            filtered_pred = [var2vals["pred"][i] for i in valid_indices]
                            vote_data = [
                                {"val": val, "pred": pred} for val, pred in zip(var_vals, filtered_pred, strict=True)
                            ]
