...
def calculate_debug_metrics(data: DataProto) -> dict:
    ...
    Returns:
        dict: metrics
            ...
            "training/vllm_k3_kl_mean": mean KL divergence using K3 formula: exp(log_ratio) - log_ratio - 1
            "training/vllm_k3_kl_max": max KL divergence using K3 formula
            "training/vllm_k3_kl_std": std of KL divergence using K3 formula
    """
    ...
    # Compute vLLM KL divergence using K3 formula: exp(log_ratio) - log_ratio - 1
    # This is always non-negative and measures divergence between actor and rollout policies
    log_ratio = actor_old_log_probs - rollout_old_log_probs
    vllm_k3_kl_matrix = torch.exp(log_ratio) - log_ratio - 1
    vllm_kl_values = torch.masked_select(vllm_k3_kl_matrix, response_mask_bool)

    # Compute KL statistics
    if vllm_kl_values.numel() > 0:
        vllm_k3_kl_mean = torch.mean(vllm_kl_values).detach().item()
        vllm_k3_kl_max = torch.max(vllm_kl_values).detach().item()
        vllm_k3_kl_std = torch.std(vllm_kl_values).detach().item() if vllm_kl_values.numel() > 1 else 0.0
    else:
        vllm_k3_kl_mean = 0.0
        vllm_k3_kl_max = 0.0
        vllm_k3_kl_std = 0.0
    ...
    return {
        ...
        "training/vllm_k3_kl_mean": vllm_k3_kl_mean,
        "training/vllm_k3_kl_max": vllm_k3_kl_max,
        "training/vllm_k3_kl_std": vllm_k3_kl_std,
    }
