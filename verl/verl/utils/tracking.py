...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            # Build settings with optional proxy and faster sync
            settings_kwargs = {}
            if config and config["trainer"].get("wandb_proxy", None):
                settings_kwargs["https_proxy"] = config["trainer"]["wandb_proxy"]

            # Allow configuring sync behavior via config or env var
            # Setting _stats_sample_rate_seconds lower can help with more frequent uploads
            if os.environ.get("WANDB_CONSOLE", None) is None:
                # Default to 'off' to reduce console spam, but allow override
    ...
    def finish(self):
        """Explicitly finish all loggers to ensure data is flushed.

        Call this before exiting to ensure all buffered data is written.
        Do not rely on __del__ as it may not run reliably with Ray.
        """
        ...
        # Clear to prevent double-finish in __del__
        self.logger.clear()

    def __del__(self):
        # Fallback cleanup - prefer calling finish() explicitly
        if "wandb" in self.logger:
            self.logger["wandb"].finish(exit_code=0)
        if "swanlab" in self.logger:
            self.logger["swanlab"].finish()
...
class _WandbLoggingAdapter:
    """Wrapper around wandb to ensure proper logging with commit and debugging."""

    def __init__(self, wandb_module):
        self.wandb = wandb_module

    def log(self, data, step):
        """Log data to wandb with explicit commit=True to ensure immediate upload."""
        # Filter out None values
        filtered_data = {k: v for k, v in data.items() if v is not None}

        if filtered_data:
            # Print debug info for training metrics
            train_keys = [k for k in filtered_data.keys() if k.startswith(("training/", "actor/", "critic/", "perf/"))]
            print(f"[wandb] step={step}, total={len(filtered_data)} metrics, train_keys={train_keys[:5]}...", flush=True)

            # commit=True is the default, but being explicit ensures data is committed
            self.wandb.log(data=filtered_data, step=step, commit=True)
