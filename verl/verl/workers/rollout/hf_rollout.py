...
class HFRollout(BaseRollout):
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        top_k = max(0, prompts.meta_info.get("top_k", self.config.top_k))  # to be compatible with vllm
        repetition_penalty = prompts.meta_info.get("repetition_penalty", self.config.repetition_penalty)
        ...
        if not do_sample:
            ...
            # do_sample -> use base config params for training
            kwargs = {
                "do_sample": True,
                "num_beams": 1,
                "top_p": top_p,
        ...
        print(f"[HFRollout] Actual sampling_params used (is_validate={is_validate}, do_sample={do_sample}): {kwargs}")
