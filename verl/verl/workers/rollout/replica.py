...
def _load_sglang():
    ...
    try:
        ...
        # Create a comprehensive mock for vllm to satisfy sglang's fallback imports
        # when CUDA is not available in the main process. Ray GPU workers will
        # re-import sglang with proper CUDA visibility and use real CUDA kernels.
        #
        # sglang imports from vllm in fallback paths (when is_cuda() returns False):
        # - vllm._custom_ops (rotary_embedding, fused_add_rms_norm, rms_norm, etc.)
        # - vllm.model_executor.layers.activation (GeluAndMul, SiluAndMul)
        ...
        mock_vllm.model_executor = Mock()
        mock_vllm.model_executor.layers = Mock()
        mock_vllm.model_executor.layers.activation = Mock()
        mock_vllm.model_executor.layers.layernorm = Mock()
        ...
        sys.modules["vllm.model_executor.layers.activation"] = mock_vllm.model_executor.layers.activation
        sys.modules["vllm.model_executor.layers.layernorm"] = mock_vllm.model_executor.layers.layernorm
