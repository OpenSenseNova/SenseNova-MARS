...
class AsyncRolloutRequest(BaseModel):
    ...
    def _get_position_ids(
        ...
        # special case for qwen vl (both qwen2vl and qwen3vl)
        is_qwen2vl = (
        ...
        if is_qwen2vl:
            ...
            processing_path = getattr(processing_class, 'name_or_path', '') or ''

            if "Qwen3VLProcessor" in processor_name:
                from verl.models.transformers.qwen3_vl import get_rope_index
            elif "qwen3" in processing_path.lower() or "Qwen3" in processing_path:
                # Qwen3 model detected but Qwen3VLProcessor not available
                raise RuntimeError(
                    f"Qwen3-VL model detected (processing path: {processing_path}) but Qwen3VLProcessor "
