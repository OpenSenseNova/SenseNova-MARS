...
def _compute_rollout_ns(index: np.ndarray | list) -> list[int]:
    """Compute rollout_n for each item based on sample index.

    Following Miles' deterministic inference pattern:
    - Consecutive items with the same sample index are rollouts of that sample
    - rollout_n increments for each consecutive duplicate, resets for new samples

    Args:
        index: Array of sample indices (from non_tensor_batch["index"])

    Returns:
        List of rollout_n values, same length as index

    Example:
        index = [0, 0, 0, 1, 1, 1, 2, 2]
        returns [0, 1, 2, 0, 1, 2, 0, 1]
    """
    rollout_ns = []
    rollout_n = 0
...
class SGLangRollout(BaseRollout):
    ...
    def _init_sampling_params(self, **kwargs):
        ...
        # NOTE: Exclude seed from base sampling params - seed is handled per-request:
        # - Training: NO seed (needs diversity for GRPO)
        # - Validation: uses val_kwargs.seed if set
        for k in self.config.keys():
            if (hasattr(SamplingParams(), str(k)) or "stop" in str(k)) and k != "seed":
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            ...
            # When validating with greedy decoding, still respect penalty settings from val_kwargs
            if is_validate:
                request_sampling_params.update(
                    {
                        "repetition_penalty": self.config.val_kwargs.repetition_penalty,
                        "presence_penalty": self.config.val_kwargs.presence_penalty,
                    }
                )
        elif is_validate:
            # Validation with sampling (do_sample=True): use seed for reproducibility
            request_sampling_params.update(
                {
            ...
            # Add seed for validation reproducibility (if configured)
            if self.config.val_kwargs.seed is not None:
                # Validate: seed with n > 1 is pointless (all rollouts would be identical)
                val_n = self.config.val_kwargs.get("n") or 1
                if val_n > 1:
                    raise ValueError(
                        f"val_kwargs.seed={self.config.val_kwargs.seed} with val_kwargs.n={val_n} is invalid. "
                        f"When seed is set, all {val_n} rollouts would be identical. "
                        f"Either remove seed (for diverse rollouts) or set n=1 (for reproducibility)."
                    )
                request_sampling_params["seed"] = self.config.val_kwargs.seed
        else:
            # Training with sampling: seed enables reproducible training
            request_sampling_params.update(
                {
                    "temperature": self.config.temperature,
                    "top_p": self.config.top_p,
            ...
            # Training seed: per-rollout variation for diversity + reproducibility
            # Will be applied per-group below if seed is configured

        # Log actual sampling params being used
        print(f"[SGLangRollout] Actual sampling_params used (is_validate={is_validate}, do_sample={do_sample}): {request_sampling_params}")
        ...
        if self._tp_rank == 0:
            ...
            # Check if we need per-rollout seeding for training
            base_seed = self.config.get("seed") if (do_sample and not is_validate) else None

            if base_seed is not None:
                # Per-rollout seeding: compute rollout_n and generate per-group
                # Following Miles' deterministic inference pattern: seed + rollout_n
                if prompts.non_tensor_batch is None or "index" not in prompts.non_tensor_batch:
                    raise ValueError(
                        "non_tensor_batch['index'] is required for per-rollout seeding when seed is configured. "
                        "Ensure dataset sets 'index' field and _get_gen_batch preserves it. "
                        "If you don't need reproducible seeding, remove 'seed' from rollout config."
                    )
                index = prompts.non_tensor_batch["index"]
                rollout_ns = _compute_rollout_ns(index)

                # Group items by rollout_n for efficient batched generation
                unique_rollout_ns = sorted(set(rollout_ns))
                all_outputs = [None] * batch_size

                for rn in unique_rollout_ns:
                    # Get indices for this rollout_n
                    group_indices = [i for i, r in enumerate(rollout_ns) if r == rn]

                    # Prepare inputs for this group
                    group_idx_list = [idx_list[i] for i in group_indices]
                    group_image_list = [image_list[i] for i in group_indices]

                    # Set seed for this group
                    group_sampling_params = request_sampling_params.copy()
                    group_sampling_params["seed"] = base_seed + rn

                    # Generate for this group
                    group_output = loop.run_until_complete(
                        self._engine.async_generate(
                            prompt=None,
                            sampling_params=group_sampling_params,
                            return_logprob=True,
                            input_ids=group_idx_list,
                            image_data=group_image_list,
                        )
                    )

                    # Store outputs in correct positions
                    for batch_idx, out in zip(group_indices, group_output):
                        all_outputs[batch_idx] = out

                output = all_outputs
            else:
                # No per-rollout seeding needed: generate all at once
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        if not do_sample:
            ...
            # Training seed: per-rollout variation for diversity + reproducibility
            # Formula: base_seed + rollout_n (following Miles' deterministic inference pattern)
            # - Different rollouts of same sample get different seeds (diversity for GRPO)
            # - Same sample + rollout_n always gets same seed across runs (reproducibility)
            if self.config.get("seed") is not None:
                request_sampling_params["seed"] = self.config.seed + rollout_n

        # Log actual sampling params being used (only log once per request to avoid spam)
        if current_turns == 0:
            print(f"[SGLangRollout._async] Actual sampling_params used (is_validate={is_validate}, do_sample={do_sample}): {request_sampling_params}")
        ...
        if self.config.calculate_log_probs:
            ...
            # Convert tensor to list of lists for SGLang batch processing
            input_ids_list = [_req.input_ids.squeeze(0).tolist()] if isinstance(_req.input_ids, torch.Tensor) else [_req.input_ids]
    ...
    async def _handle_engine_generate(
        ...
        # Wrap in list for SGLang batch processing (expects list of lists for input_ids,
        # and list of image_data to match batch size)
        output = await self._engine.async_generate(
            input_ids=[generation_prompt_ids],
            sampling_params=kwargs,
            return_logprob=return_logprob,
            image_data=[image_data] if image_data is not None else None,
        )
        return output
    ...
    def _inject_image_search_into_prompt(
        ...
        This is the equivalent of directly_provide_image_search=True in the old codebase.
        Instead of requiring the model to call image_search_tool(), the search results
        are pre-injected into the user prompt.

        Args:
            raw_prompt: List of message dicts
            multi_modal_data: Dict with 'image' list (can be None)
            tools_kwargs: Contains image_search_tool.create_kwargs with title/thumbnail lists

        Returns:
            Tuple of (modified raw_prompt, modified multi_modal_data)
        """
        ...
        if not title_list or not thumbnail_list:
            return raw_prompt, multi_modal_data

        # Deep copy messages to avoid mutating original data
        # (messages are dicts/strings, safe to deepcopy)
        raw_prompt = deepcopy(raw_prompt)

        # For multi_modal_data, do a shallow copy of the dict and copy the image list
        # (avoid deepcopy on PIL images which is slow and can cause issues)
        if multi_modal_data is None:
            multi_modal_data = {"image": [], "video": []}
        else:
            # Shallow copy the dict
            multi_modal_data = copy(multi_modal_data)
            # Copy the image list (not the images themselves, just the list reference)
            if "image" in multi_modal_data:
                multi_modal_data["image"] = list(multi_modal_data["image"])
            else:
        ...
        # Build image search content (aligned with old code's _build_image_search_content)
        content_to_add = [
            {
                "type": "text",
            ...
            if i < len(thumbnail_list):
                ...
                try:
                    if os.path.exists(thumbnail_path):
                        ...
                        # Add actual image to multi_modal_data
                        multi_modal_data["image"].append(thumbnail_img)
                    else:
        ...
        user_content = raw_prompt[last_user_idx].get("content", "")
        if isinstance(user_content, str):
            # Convert string content to list format
            raw_prompt[last_user_idx]["content"] = [{"type": "text", "text": user_content}] + content_to_add
        elif isinstance(user_content, list):
            # Extend existing list content (already copied via deepcopy of raw_prompt)
            raw_prompt[last_user_idx]["content"] = raw_prompt[last_user_idx]["content"] + content_to_add
        else:
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._tp_rank == 0:
            ...
            # Compute rollout_n for per-rollout seeding (following Miles' pattern)
            batch_size = len(req_list)
            if prompts.non_tensor_batch is not None and "index" in prompts.non_tensor_batch:
                index = prompts.non_tensor_batch["index"]
            else:
                # Only error if seed is configured for training (when rollout_n is actually used)
                if not is_validate and self.config.get("seed") is not None:
                    raise ValueError(
                        "non_tensor_batch['index'] is required for per-rollout seeding when seed is configured. "
                        "Ensure dataset sets 'index' field and _get_gen_batch preserves it. "
                        "If you don't need reproducible seeding, remove 'seed' from rollout config."
                    )
                # Fallback for validation or when no seed configured (rollout_n not used for seeding)
            ...
            if is_validate:
                ...
                # rollout_n is passed but ignored inside _async_rollout_a_request for validation
                # (validation uses val_kwargs.seed directly, not seed + rollout_n)
                loop = asyncio.get_event_loop()
                output_req_list = loop.run_until_complete(
                    asyncio.gather(
                        *[self._async_rollout_a_request(req, do_sample, is_validate, rollout_n=rn, **kwargs)
                          for req, rn in zip(req_list, rollout_ns)],
                    )
                )
            else:
                ...
                async def rollout_a_request_with_cancellation_handler(req, rn):
                    try:
                        result = await self._async_rollout_a_request(req, do_sample, is_validate, rollout_n=rn, **kwargs)
                        return result
                ...
                async def run_with_cancellation():
                    ...
                    all_tasks = [
                        asyncio.create_task(rollout_a_request_with_cancellation_handler(req, rn))
                        for req, rn in zip(req_list, rollout_ns)
                    ]
        ...
        if is_multimodal:
            # Force 1D array to avoid dimension mismatch when concatenating across workers
            # (np.array() creates 2D if all elements have same shape, 1D otherwise)
            arr = np.empty(len(multi_modal_inputs), dtype=object)
            arr[:] = multi_modal_inputs
    ...
    def _preprocess_prompt_to_async_rollout_requests(self, prompts: DataProto, n: int = 1) -> list[AsyncRolloutRequest]:
        ...
        for data_idx, (raw_prompt, multi_modal_data) in enumerate(
            ...
            if self.config.multi_turn.directly_provide_image_search and _tools_kwargs:
                raw_prompt, multi_modal_data = self._inject_image_search_into_prompt(
                    list(raw_prompt), multi_modal_data, _tools_kwargs
                )
...
class ServerAdapter(BaseRollout):
    ...
    async def _init_server_adapter(self):
        ...
        # Pass attention_backend to suppress misleading warning from client-side ServerArgs.
        # The actual server already uses FA3 (set in async_sglang_server.py:158-159).
        # Note: These values only affect the client-side ServerArgs object, not the running server.
        engine_kwargs = self.config.get("engine_kwargs", {}).get("sglang", {}) or {}
        # Use `or "fa3"` to handle YAML null values (key exists but value is None)
        attention_backend = engine_kwargs.get("attention_backend") or "fa3"
        mm_attention_backend = engine_kwargs.get("mm_attention_backend") or "fa3"
