...
try:
    ...
    # vLLM 0.11.x moved get_tcp_uri to network_utils
    from vllm.utils.network_utils import get_tcp_uri
try:
    from vllm.utils import FlexibleArgumentParser
except ImportError:
    # vLLM 0.11.x moved FlexibleArgumentParser to argparse_utils
    try:
...
class ExternalZeroMQDistributedExecutor(Executor):
    """An executor that communicates with external Ray actor workers via ZeroMQ.

    This executor is used when vLLM inference workers are managed externally
    (e.g., by verl's training framework) rather than spawned by vLLM itself.

    Supports both vLLM V0 (synchronous) and V1 (async with non_block=True).
    See: https://github.com/volcengine/verl/pull/2246
    """

    uses_ray: bool = False

    def _init_executor(self) -> None:
        """Initialize ZMQ connections to worker actors and load the model."""
        logger.info("[Executor] _init_executor: Setting up ZMQ connections...")
        ...
        logger.info(f"[Executor] _init_executor: Connected to {len(self.sockets)} worker(s)")
        ...
        logger.info("[Executor] _init_executor: Initializing workers...")
        self.collective_rpc("init_worker", args=([kwargs],))
        logger.info("[Executor] _init_executor: Initializing devices...")
        self.collective_rpc("init_device")
        logger.info("[Executor] _init_executor: Loading model (this may take a while)...")
        self.collective_rpc("load_model")
        logger.info("[Executor] _init_executor: Model loaded successfully")
    ...
    def collective_rpc(
        ...
        **kwargs_extra: Any,
    ) -> Union[list[Any], "concurrent.futures.Future[list[Any]]"]:
        """Execute RPC call to all workers via ZeroMQ.

        Args:
            method: Method name (str) or callable to execute on workers.
            timeout: Optional timeout in seconds (not currently implemented).
            args: Positional arguments for the method.
            kwargs: Keyword arguments for the method.
            non_block: If True, return a Future for async execution (vLLM V1).
                      If False, block until all workers respond (vLLM V0).
            **kwargs_extra: Additional kwargs (ignored, for API compatibility).

        Returns:
            If non_block=False: List of outputs from all workers.
            If non_block=True: Future that resolves to list of outputs.

        Note:
            ZMQ REQ sockets enforce send-recv-send-recv pattern. The vLLM V1
            engine guarantees it waits for previous future before next call,
            so concurrent non_block calls are safe.
        """
        ...
        if non_block:
            # vLLM V1 async execution: return Future that resolves when recv completes
            future: concurrent.futures.Future = concurrent.futures.Future()
            sockets = self.sockets  # Capture reference for thread closure

            def _recv_and_set_result():
                try:
                    outputs = []
                    for sock in sockets:
                        outputs.append(pickle.loads(sock.recv()))
                    # Check if any worker returned an exception
            ...
            thread = threading.Thread(target=_recv_and_set_result, daemon=True)
            thread.start()
            return future
        else:
            # vLLM V0 synchronous execution: block until all workers respond
            outputs = []
            for socket in self.sockets:
    ...
    def execute_model(self, scheduler_output, non_block: bool = False):
        ...
        Overrides base class to properly support vLLM V1 async execution.
        V1 engine calls this with non_block=True and expects a Future.

        Args:
            scheduler_output: SchedulerOutput containing the batch to process.
            non_block: If True, return Future (V1). If False, block (V0).

        Returns:
            If non_block=False: ModelRunnerOutput from the first worker.
            If non_block=True: Future[ModelRunnerOutput] for async execution.
        """
        ...
        if non_block:
            # Transform Future[list] -> Future[first_element]
            # V1 engine expects Future[ModelRunnerOutput], not Future[list]
            result_future: concurrent.futures.Future = concurrent.futures.Future()

            def _on_complete(fut: concurrent.futures.Future):
                try:
                    outputs = fut.result()
                    result_future.set_result(outputs[0] if outputs else None)
                except Exception as e:
                    result_future.set_exception(e)

            output.add_done_callback(_on_complete)
            return result_future
        else:
            # Synchronous: extract first worker's output
            return output[0] if output else None

    def check_health(self):
        """Health check - always returns OK for external workers."""
        return
...
class vLLMHttpServerBase:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        # 3. launch server
        logger.info(f"[replica={self.replica_rank}] launch_server: Starting server (node_rank={self.node_rank})...")
        if self.node_rank == 0:
            await self.run_server(server_args)
        else:
            await self.run_headless(server_args)
        logger.info(f"[replica={self.replica_rank}] launch_server: Server launched successfully")

    async def run_server(self, args: argparse.Namespace):
        logger.info(f"[replica={self.replica_rank}] run_server: Creating engine args...")
        engine_args = AsyncEngineArgs.from_cli_args(args)
        usage_context = UsageContext.OPENAI_API_SERVER
        logger.info(f"[replica={self.replica_rank}] run_server: Creating engine config...")
        vllm_config = engine_args.create_engine_config(usage_context=usage_context)
        vllm_config.parallel_config.data_parallel_master_port = self._dp_master_port

        logger.info(f"[replica={self.replica_rank}] run_server: Initializing AsyncLLM engine (this may take a while)...")
        import time
        init_start_time = time.time()
        # vLLM API compatibility: handle disable_log_requests removal in 0.12.0+
        # Old vLLM (<=0.11.x): uses disable_log_requests/disable_log_stats
        # New vLLM (>=0.12.0): logging disabled by default, removed these attributes
        async_llm_kwargs = dict(
            vllm_config=vllm_config,
            usage_context=usage_context,
        )
        if hasattr(engine_args, 'disable_log_requests'):
            async_llm_kwargs['disable_log_requests'] = engine_args.disable_log_requests
        if hasattr(engine_args, 'disable_log_stats'):
            async_llm_kwargs['disable_log_stats'] = engine_args.disable_log_stats

        engine_client = AsyncLLM.from_vllm_config(**async_llm_kwargs)
        init_elapsed = time.time() - init_start_time
        logger.info(f"[replica={self.replica_rank}] run_server: AsyncLLM engine initialized successfully in {init_elapsed:.1f}s")

        # Don't keep the dummy data in memory
        logger.info(f"[replica={self.replica_rank}] run_server: Resetting multimodal cache...")
        await engine_client.reset_mm_cache()

        logger.info(f"[replica={self.replica_rank}] run_server: Building app and initializing state...")
        app = build_app(args)
        # vLLM API compatibility: check init_app_state signature
        # Old vLLM: init_app_state(engine_client, vllm_config, state, args) - 4 args
        # New vLLM 0.11.x: init_app_state(engine_client, state, args) - 3 args
        import inspect
        sig = inspect.signature(init_app_state)
        if len(sig.parameters) == 4:
            # Old vLLM API with vllm_config
            await init_app_state(engine_client, vllm_config, app.state, args)
        else:
            # New vLLM 0.11.x API without vllm_config
            await init_app_state(engine_client, app.state, args)
        if self.replica_rank == 0 and self.node_rank == 0:
            logger.info(f"Initializing a V1 LLM engine with config: {vllm_config}")

        self.engine = engine_client
        logger.info(f"[replica={self.replica_rank}] run_server: Starting HTTP server...")
        self._server_port, self._server_task = await run_unvicorn(app, args, self._server_address)
        logger.info(f"[replica={self.replica_rank}] run_server: HTTP server started on port {self._server_port}")
    ...
    async def generate(
        ...
        sampling_params.setdefault("presence_penalty", self.config.presence_penalty)
