...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            # Greedy decoding: deterministic temp/top_p/top_k, but use kwargs for penalties
            if is_validate:
                kwargs = {
                    "best_of": 1,
                    "top_p": 1.0,
            ...
            if self.config.val_kwargs.seed is not None:
                ...
                kwargs["seed"] = self.config.val_kwargs.seed
        else:
            # Training with sampling: NO seed (need diversity for GRPO)
            kwargs = {
            ...
            # Warn if user mistakenly set seed for training
            if self.config.get("seed") is not None:
                logger.warning(
                    f"[vLLMRolloutSPMD] seed={self.config.seed} is configured for training but will be IGNORED. "
                    "Training requires random sampling for GRPO diversity. "
                    "Seeds are only supported for validation (val_kwargs.seed)."
                )

        # Log actual sampling params being used
        print(f"[vLLMRolloutSPMD] Actual sampling_params used (is_validate={is_validate}, do_sample={do_sample}): {kwargs}")
